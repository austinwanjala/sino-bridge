...
export default async function NewFee() {
  ...
  async function createFee(formData: FormData) {
    ...
    await sb.from('fees').insert(newFee)
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Create a new pricing tier or fee.</p>
      </div>

      <form action={createFee} className="bg-white shadow rounded-lg p-6 space-y-4">
        
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Fee Title *</label>
            <input type="text" name="title" required placeholder="e.g. Standard Tuition" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md" />
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" name="amount" required placeholder="e.g. $500 or ¥3500" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Billing Cycle</label>
            <input type="text" name="billing_cycle" placeholder="e.g. Per Term, Monthly, One-time" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md" />
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700">Description / What's Included</label>
          <textarea name="description" rows={3} placeholder="Includes materials, textbooks, etc." className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md"></textarea>
        </div>
